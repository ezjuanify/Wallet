...
func (s *WithdrawService) DoWithdraw(ctx context.Context, tx *sql.Tx, username string, amount int64) (*model.Wallet, error) {
	...
	if currentWallet == nil {
		return nil, fmt.Errorf("%s - user %s does not have a wallet", fnName, username)
	}
	...
	if err := validation.ValidateWalletBalance(newBalance); err != nil {
		...
		return nil, fmt.Errorf("%s - insufficient wallet balance - balance: %d - withdraw: %d - overdraft: %d", fnName, currentWallet.Balance, amount, newBalance)
	}
	logger.Info(
	...
	logger.Info(fmt.Sprintf("%s - Withdrawn wallet", fnName), zap.Any("wallet", updatedWallet))
